...
namespace TheXDS.Proteus.Api
{
    ...
    public abstract class Service : Plugin, IDisposable, IFullService
    {
        ...
        public IEnumerable<KeyValuePair<MethodInfo, SecurityFlags>> Functions
        {
            get
            {
                foreach (var j in GetType().GetMethods())
                {
                    ...
                    yield return new KeyValuePair<MethodInfo, SecurityFlags>(
                        j, kind.Value);
                }
            }
        }
        ...
        ///     de una operación.
        /// </summary>
        public IStatusReporter Reporter { get => _reporter ?? CommonReporter; set => _reporter = value; }
    ...
    public abstract class Service<TContext, TSettings> : Service<TContext>, ISettingsRepository where TContext : DbContext, new() where TSettings : class, ISettingsRepository, new()
    {
        ...
        ///     servicio.
        /// </summary>
        public static TSettings SettingsRepo { get; } = Objects.FindSingleObject<TSettings>();
